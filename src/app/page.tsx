...
export default function Home() {
  ...
  return (
    <div style={{ minHeight: '100vh', padding: '32px 16px' }}>
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          <p className="heading-sub">PERSONAL PLANNER</p>
          <h1 className="heading-display" style={{ fontSize: '1.8rem', marginTop: '2px' }}>DT-Günlüğüm 📒</h1>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem', marginTop: '4px' }}>
            {dayName}, {dateStr} · Hoşgeldin, <strong>{user?.full_name || 'Kullanıcı'}</strong>
          </p>
        </div>
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          <button className="btn-theme" onClick={() => setTheme(isDark ? 'light' : 'dark')}>
